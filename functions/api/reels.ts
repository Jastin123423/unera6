...
/**
 * POST /api/reels
 * Supports: audio_url, audio_start, audio_end, visibility, location, song_id, sound_key
...
export const onRequestPost: PagesFunction<Env> = async ({ request, env }) => {
  try {
    ...
    // ✅ NEW: persist sound identity for reuse + sounds endpoints
    ...
    // ✅ UPDATED: insert song_id + sound_key (your table already has these columns)
    const result = await env.DB.prepare(
      `INSERT INTO reels
        (user_id, video_url, caption, song_name, audio_url, audio_start, audio_end, song_id, sound_key, visibility, location, views, shares)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, 0)`
    )
      .bind(
        user_id,
 ...
 * plus counts + my_reaction and the new fields (audio/visibility/location/shares/views/song_id/sound_key)
 */
  ...
  try {
    ...
    const out = reels.map((r: any) => {
      ...
      return {
        ...
        song_id: (r as any).song_id == null ? null : toNum((r as any).song_id, 0),
        sound_key: String((r as any).sound_key ?? ''),
