...
const normalizePostMedia = (item: any) => {
  const meta = normalizeMedia(item).media;
...
/**
 * GET /api/posts
 * Like feeds.ts for posts + group posts only
 * No reels, no video posts
 */
export const onRequestGet: PagesFunction<Env> = async ({ request, env }) => {
  try {
    ...
    const perType = clamp(Math.ceil(limit * (isGuest ? 2.5 : 1.75)), 12, 120);

    // ============================================================
    // POSTS — same style as feeds.ts
    // ============================================================
    const qPosts = `
      SELECT
        'post' AS source,
      ...
      WHERE
        (p.visibility IS NULL OR TRIM(LOWER(p.visibility)) = 'public' OR TRIM(p.visibility) = '')
        AND (
    ...
    // ============================================================
    // GROUP POSTS — same style as feeds.ts
    // ============================================================
    const qGroupPosts = `
      SELECT
      ...
      WHERE
        (gp.visibility IS NULL OR TRIM(LOWER(gp.visibility)) = 'public' OR TRIM(gp.visibility) = '')
