...
const guessTypeFromUrl = (url: string) => {
  ...
  if (u.includes(".mp3") || u.includes(".wav") || u.includes(".m4a") || u.includes("audio")) return "audio";
  return "image";
};
...
const normalizeMedia = (row: any) => {
  ...
  // media_types is not urls; but our parseJsonArray works fine (strings)
...
export const onRequestPost: PagesFunction<Env> = async ({ request, env }) => {
  try {
    if (!env.DB)
      return json({ error: "D1 binding missing. Set Pages D1 binding name to DB." }, 500);
...
/* ============================================================
   ...
   ✅ returns media_urls/media_types as ARRAYS + images alias
   ✅ includes comments_count for posts
============================================================ */
...
export const onRequestGet: PagesFunction<Env> = async ({ request, env }) => {
  try {
    ...
    /* ============================================================
       GROUP POSTS ✅ reactions + gp.media_urls
       comments_count: 0 (until you add group comments table)
    ============================================================ */
    ...
    const merged = mergedRaw.map((it: any) => ({
      ...it,
      ...normalizeMedia(it),
      comments_count: Number((it as any)?.comments_count ?? 0),
    }));
