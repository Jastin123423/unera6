...
export const onRequest: any = async (context: any) => {
  ...
  if (request.method === 'POST') {
    try {
      ...
      return new Response(JSON.stringify({ ...newPost, reactions: [], comments: [], shares: 0 }), { status: 201, headers: jsonHeaders });
    } catch (e) { return new Response(JSON.stringify({ error: "Unauthorized" }), { status: 401, headers: jsonHeaders }); }
  }
  ...
  if (request.method === 'GET') {
    try {
      ...
      // Ensure results are sanitized for React components
      const sanitized = (results || []).map((p: any) => ({
        ...p,
        reactions: [], // In a real app, join these or fetch on demand
        comments: [],
        shares: p.shares || 0
      }));
