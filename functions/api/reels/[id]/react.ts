// functions/api/reels/[id]/react.ts
 ...
 * Body: { user_id, type }
 * Toggles reaction or updates reaction type
 * Returns: { success, liked, likes_count, type }
 */
  ...
  try {
    ...
    if ((existing as any)?.id) {
      ...
      if (existingType === type) {
        ...
        await env.DB.prepare(
          `UPDATE reel_likes SET type = ? WHERE reel_id = ? AND user_id = ?`
        )
