...
export const StoryViewer: React.FC<StoryViewerProps> = ({
  ...
  // ✅ ADDED: Reset media failed state when story changes
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (storyType !== 'video' && !isVideoUrl(url)) {
      const img = new Image();
      img.src = url;
  ...
  }, [story.id, storyType, mediaUrl]);

  // ✅ ADDED: Video ready state detection
  useEffect(() => {
    if (storyType === 'video' || isVideoUrl(mediaUrl)) {
  ...
  useEffect(() => {
    ...
    setMediaReady(storyType === 'text');
  }, [story.id, story.user_id, allStories, storyType]); // ✅ Added allStories dependency
  ...
  const storyIsText = storyType === 'text';
  const storyIsVideo = storyType === 'video' || (!storyIsText && isVideoUrl(mediaUrl));
    ...
    <div className="fixed inset-0 z-[250] bg-black flex items-center justify-center animate-fade-in">
      <div
        ...
        style={{
          ...
          background: !mediaUrl ? storyBg : undefined,
        }}
      />
      ...
      >
            ...
            >
              ...
              <div className="flex flex-col items-start">
                ...
                <span className="text-white/70 text-[12px] drop-shadow-md">
                  {formatStoryTime(storyCreatedAt)}
                </span>
              </div>
            </button>
        ...
        {/* ✅ UPDATED: Content area with proper tap zones and media failure fallback */}
        <div className="flex-1 bg-[#111] relative">
          ...
          <div className="absolute inset-0 z-[10] flex items-center justify-center">
            ...
            ) : mediaUrl && !isBlob(mediaUrl) && !mediaFailed ? (
              storyIsVideo ? (
                <video
                  ref={videoRef}
                  src={mediaUrl}
              ...
              <div className="w-full h-full flex items-center justify-center bg-[#111] px-6">
                <div className="text-center max-w-[340px]">
                  ...
                  <div className="mt-4 text-left bg-white/5 border border-white/10 rounded-xl p-3 text-xs text-white/70 space-y-1">
                    <div><span className="text-white">id:</span> {String(story.id || 'none')}</div>
                    <div><span className="text-white">type:</span> {String(storyType || 'none')}</div>
                    <div><span className="text-white">mediaUrl:</span> {String(mediaUrl || 'none')}</div>
                    <div><span className="text-white">text:</span> {String(storyText || 'none')}</div>
                    <div><span className="text-white">failed:</span> {String(mediaFailed)}</div>
                  </div>
                </div>
              </div>
            )}
...
export const CreateStoryModal: React.FC<CreateStoryModalProps> = ({
  ...
  // ... (CreateStoryModal implementation remains the same as previous) ...
