//Feed.tsx (Updated with PostMenu component and Reel edit/delete + Notification System)
...
// ==================== PROPS INTERFACE WITH NOTIFICATION SYSTEM ====================
interface FeedProps {
...
/**
 * =========================
 * ✅ MAIN FEED COMPONENT WITH NOTIFICATION SYSTEM
 * =========================
 */
const Feed: React.FC<FeedProps> = ({
  ...
  const fetchFeed = useCallback(async (pageNum: number, refresh = false) => {
    ...
    try {
      ...
      const res = await fetch(`/api/feeds?page=${pageNum}&limit=10`, {
        headers: {
      ...
      const unwrapped = items.map(unwrapFeedItem).filter(Boolean);

      setFeedItems(prev => {
        if (refresh || pageNum === 1) return unwrapped;
        return [...prev, ...unwrapped];
      });
  ...
  const fetchSuggestions = useCallback(async () => {
    ...
    try {
      const [peopleRes, groupsRes, productsRes] = await Promise.all([
        fetch('/api/suggestions/people', {
          headers: { 'x-user-id': String(currentUser.id) }
        }),
        fetch('/api/suggestions/groups', {
          headers: { 'x-user-id': String(currentUser.id) }
        }),
      ...
      setPeopleSuggestions(Array.isArray(peopleData) ? peopleData : []);
      setGroupSuggestions(Array.isArray(groupsData) ? groupsData : []);
      setSuggestedProducts(Array.isArray(productsData) ? productsData : []);
    } catch (err) {
  ...
  // ========== AUTO REFRESH (Facebook style) ==========
  useEffect(() => {
    if (!currentUser) return;

    const refreshFeed = () => {
      setRefreshing(true);
      fetchFeed(1, true).then(() => {
        // After feed refresh, also refresh suggestions quietly
        fetchSuggestions();
      });
    };

    // Initial refresh after 30 seconds
    refreshTimeoutRef.current = setTimeout(refreshFeed, 30000);

    // Set up interval for continuous refresh
    const interval = setInterval(refreshFeed, 60000); // Every minute
  ...
  // ========== HANDLE POST REACTION WITH NOTIFICATION ==========
  const handleReact = useCallback(async (postId: number, type: ReactionType) => {
    if (!currentUser) {
    ...
    const post = feedItems.find(item => Number(item.id) === Number(postId));
    if (!post) return;
    ...
    setFeedItems(prev => prev.map(item => {
      ...
      const myReaction = item.myReaction || item.my_reaction;
      const reactionsCount = Number(item.reactionsCount || item.reactions_count || 0);

      let newMyReaction = type;
      let newCount = reactionsCount;

      if (myReaction === type) {
        // Remove reaction
        newMyReaction = null;
        newCount = Math.max(0, reactionsCount - 1);
      } else {
        // Add/change reaction
        newCount = myReaction ? reactionsCount : reactionsCount + 1;
        newMyReaction = type;
      }
    ...
    try {
      ...
      // ========== CREATE NOTIFICATION ==========
      // Only notify if reacting to someone else's post and not removing reaction
      if (postOwnerId !== currentUser.id) {
        // Check if this is a new reaction (not removing)
        const previousReaction = post.myReaction || post.my_reaction;
        if (previousReaction !== type) {
          await createNotification(
            ...
            'post',
            String(postId),
            `${currentUser.name || 'Someone'} reacted to your post`
          );
        }
      }
    } catch (error) {
  ...
  }, [currentUser, feedItems, createNotification, fetchFeed]);

  // ========== HANDLE COMMENT WITH NOTIFICATION ==========
  const handleComment = useCallback(async (postId: number, text: string) => {
    if (!currentUser) return;
    ...
    try {
      ...
      if (postOwnerId !== currentUser.id) {
        await createNotification(
          ...
          'post',
          String(postId),
          `${currentUser.name || 'Someone'} commented on your post: "${text.slice(0, 50)}${text.length > 50 ? '...' : ''}"`
        );
      }
  ...
  }, [currentUser, feedItems, createNotification, onOpenComments]);

  // ========== HANDLE SHARE WITH NOTIFICATION ==========
  const handleShare = useCallback(async (postId: number, destination: string) => {
    if (!currentUser) return;
    ...
    setFeedItems(prev => prev.map(item => {
      ...
      const currentShares = Number(item.shares || item.shares_count || 0);
      return {
        ...item,
    ...
    try {
      ...
      // ========== CREATE NOTIFICATION ==========
      // Only notify if sharing someone else's post
      if (postOwnerId !== currentUser.id) {
        await createNotification(
          postOwnerId,
          'share',
          'post',
          String(postId),
          `${currentUser.name || 'Someone'} shared your post`
        );
      }
      ...
      setFeedItems(prev => prev.map(item => {
        ...
        return {
          ...item,
          shares: Math.max(0, currentShares - 1),
          shares_count: Math.max(0, currentShares - 1),
        };
      }));
    }
  }, [currentUser, feedItems, createNotification]);
  ...
  const handleFollow = useCallback(async (userId: number) => {
    ...
    try {
      ...
      // ========== CREATE NOTIFICATION ==========
      // Only notify if following someone else
  ...
  }, [currentUser, createNotification, onFollow, fetchSuggestions]);

  // ========== HANDLE GROUP JOIN WITH NOTIFICATION ==========
  const handleJoinGroup = useCallback(async (groupId: number) => {
    ...
    try {
      ...
      // Find group to get admin ID for notification
      const group = groupSuggestions.find(g => g.id === groupId);
      if (group && group.admin_id !== currentUser.id) {
        await createNotification(
          ...
          'group',
          String(groupId),
          `${currentUser.name || 'Someone'} joined your group "${group.name}"`
        );
      }
  ...
  }, [currentUser, groupSuggestions, createNotification, fetchSuggestions]);
  ...
  const handleCreatePost = useCallback(async (
    ...
    if (meta?.feeling) formData.append('feeling', meta.feeling);
    if (meta?.background) formData.append('background', meta.background);
    if (meta?.linkPreview) formData.append('link_preview', JSON.stringify(meta.linkPreview));
  ...
  const handleRSVP = useCallback(async (
    ...
    try {
      ...
      // Find event to get creator ID for notification
      const eventItem = feedItems.find(item => 
        (item.event_id === eventId || item.id === eventId) && 
        (item.type === 'event' || item.post_type === 'event')
      );
      
      if (eventItem && res?.success) {
        const creatorId = Number(eventItem.creator_id || eventItem.user_id);
        
        // ========== CREATE NOTIFICATION ==========
        // Only notify if RSVPing to someone else's event and not "not_going"
        if (creatorId !== currentUser.id && status !== 'not_going') {
          await createNotification(
            creatorId,
            'event_rsvp',
            'event',
            String(eventId),
            `${currentUser.name || 'Someone'} is ${status} to your event`
          );
        }
      }
  ...
  }, [currentUser, onRSVP, feedItems, createNotification, fetchFeed]);
  ...
  if (!currentUser) {
    return (
      <div className="flex-1 bg-[#18191A] min-h-screen flex items-center justify-center">
        <div className="text-center p-8">
          <h2 className="text-[#E4E6EB] text-2xl font-bold mb-4">Welcome to UNERA</h2>
          <p className="text-[#B0B3B8] mb-6">Please log in to see your feed</p>
          <button
    ...
    return (
      <div className="flex-1 bg-[#18191A] min-h-screen flex items-center justify-center">
        <div className="text-center">
          <i className="fas fa-spinner fa-spin text-[#1877F2] text-4xl mb-4"></i>
  ...
  if (error && feedItems.length === 0) {
    return (
      <div className="flex-1 bg-[#18191A] min-h-screen flex items-center justify-center p-4">
        <div className="text-center max-w-md">
          <i className="fas fa-exclamation-circle text-red-500 text-5xl mb-4"></i>
          <h3 className="text-[#E4E6EB] text-xl font-bold mb-2">Failed to load feed</h3>
          <p className="text-[#B0B3B8] mb-6">{error}</p>
          <button
            onClick={() => fetchFeed(1, true)}
            className="bg-[#1877F2] hover:bg-[#166FE5] text-white font-bold py-2 px-6 rounded-lg transition-colors"
          >
            Try again
          </button>
        </div>
  ...
  return (
    <div 
      ref={scrollContainerRef}
      className="flex-1 bg-[#18191A] min-h-screen overflow-y-auto"
    >
      <div className="max-w-[680px] mx-auto py-4">
        ...
        {refreshing && (
          <div className="text-center py-2 mb-2">
            <i className="fas fa-spinner fa-spin text-[#1877F2] mr-2"></i>
            <span className="text-[#B0B3B8] text-sm">Updating feed...</span>
          </div>
        )}
        ...
        <CreatePost
          ...
          onPhotoClick={() => {
            ...
            // Could trigger file input directly
          }}
          onVideoClick={() => {
            // Open recorder
            if (onVideoClick) {
        ...
        {suggestedProducts.length > 0 && (
          <SuggestedProductsWidget
            ...
            onSeeAll={() => window.location.href = '/marketplace'}
          />
        )}
        ...
        {feedItems.length === 0 ? (
          <div className="bg-[#242526] rounded-xl p-8 text-center">
            <i className="fas fa-newspaper text-4xl text-[#3A3B3C] mb-4"></i>
            <h3 className="text-[#E4E6EB] text-xl font-bold mb-2">Your feed is empty</h3>
            <p className="text-[#B0B3B8] mb-6">Follow people and join groups to see content here</p>
            <button
          ...
          feedItems.map((item, index) => {
            ...
            if (isReelPost(item)) {
              const reelData = normalizeReelFromFeed(item);
              return (
                <ReelFeedCard
                  key={`reel-${reelData.id}-${index}`}
                  reel={reelData}
                  onOpen={(reelId) => {
                    // Open full reels player
                    window.location.href = `/reels?reel=${reelId}`;
                  }}
                  onProfileClick={onProfileClick}
                />
              );
            ...
            if (item.type === 'event' || item.post_type === 'event' || item.item_type === 'event') {
              const eventData = normalizeEventFromFeed(item);
              return (
                <EventPost
                  key={`event-${eventData.id}-${index}`}
                  event={eventData}
                  ...
                  onRSVP={handleRSVP}
                  onFollow={handleFollow}
                  isFollowing={checkIsFollowing ? checkIsFollowing(eventData.creator_id) : false}
                  groups={groups}
                  brands={brands}
                  onEventClick={onEventClick}
                />
            ...
            return (
              <Post
                ...
                onEdit={(id, content) => console.log('Edit post:', id, content)}
                onViewImage={(url) => onViewImage?.(url)}
                onOpenComments={handleOpenComments}
                onVideoClick={(p) => onVideoClick?.(p)}
                ...
                brands={brands}
                chats={[]}
                isFollowing={checkIsFollowing ? checkIsFollowing(item.user_id) : false}
                onFollow={handleFollow}
                onEventClick={onEventClick}
      ...
      {/* Comments Sheet */}
      {commentsPostId && selectedPostForComments && (
        <CommentsSheet
