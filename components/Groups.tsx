...
interface GroupsPageProps {
    ...
    onPostToGroup: (groupId: number, content: string, file: File | null, type: 'image' | 'video' | 'doc' | 'text', background?: string) => void;
...
export const GroupsPage: React.FC<GroupsPageProps> = ({ currentUser, groups, users, onCreateGroup, onJoinGroup, onLeaveGroup, onDeleteGroup, onUpdateGroupImage, onPostToGroup, onCreateGroupEvent, onInviteToGroup, onProfileClick, onLikePost, onOpenComments, onSharePost, onDeleteGroupPost, onRemoveMember, onUpdateGroupSettings, initialGroupId, onPlayAudioTrack }) => {
    ...
    useEffect(() => { if (initialGroupId) { const group = groups.find(g => g.id === parseInt(initialGroupId, 10)); if (group) { setActiveGroupId(group.id); setView('detail'); setGroupTab('Discussion'); } } }, [initialGroupId, groups]);
    ...
    const handleCreateSubmit = () => { if (!newGroupName.trim()) return; onCreateGroup({ name: newGroupName, description: newGroupDesc, type: newGroupType, profile_image: `https://ui-avatars.com/api/?name=${newGroupName}&background=random`, cover_image: 'https://images.unsplash.com/photo-1522202176988-66273c2fd55f?ixlib=rb-1.2.1&auto=format&fit=crop&w=1500&q=80' }); setShowCreateModal(false); setNewGroupName(''); setNewGroupDesc(''); };
                            ...
                            {activeGroup.type === 'private' && !isMember ? (
                                ...
                                mergedPosts.length > 0 ? mergedPosts.map(post => (
                                    <Post 
                                        ...
                                        author={users.find(u => u.id === (post as any).user_id) || { id: 0, username: 'guest', name: 'Guest User', profile_image_url: 'https://ui-avatars.com/api/?name=User&background=random', followers: [], following: [], email: '' } as User}
